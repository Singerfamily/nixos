      ...
      { pkgs, ... }:
      {
        services.sssd = {
          ...
          config = ''
            ...
            # Enumerate the directory so tools that list accounts (login
            # greeters, `getent passwd`, group-membership checks) see LDAP
            # users without an explicit per-name lookup. Acceptable here: the
            # directory is small and the outpost is on loopback.
            enumerate = true
          '';
        };

        # Prefer the local outpost and the OpenBao agent (which renders the
        # LDAP bind credentials into conf.d) to be up first. Soft ordering
        # only — SSSD's credential cache must still let logins through if
        # either is down, and the agent reloads sssd once fresh creds land.
        systemd.services.sssd = {
          after = [
            "docker-authentik-ldap.service"
