{ lib, ... }:
{
  ...
  #   • any secrets declared via openbao.secrets / home-manager openbao.secrets
  #     (see modules/aspects/services/openbao-secrets.nix)
  #
  ...
  den.aspects.openbao-agent = {
    nixos =
      ...
      in
      {
        # Other modules (openbao-secrets.nix) append their generated templates
        # here so the agent picks them up without requiring direct knowledge of
        # the vault-agent option internals.
        options.openbao.extraTemplates = lib.mkOption {
